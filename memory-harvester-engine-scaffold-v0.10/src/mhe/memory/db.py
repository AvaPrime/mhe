...
async def init_db():
    ...
    async with engine.begin() as conn:
        ...
        # Hybrid search: ensure tsvector column & GIN index for messages
        await conn.execute(text("""
            ALTER TABLE mhe.message
            ADD COLUMN IF NOT EXISTS content_tsv tsvector
            GENERATED ALWAYS AS (to_tsvector('english', content)) STORED;
        """))
        await conn.execute(text(
            "CREATE INDEX IF NOT EXISTS idx_message_tsv ON mhe.message USING GIN (content_tsv)"))
