            ...
            <div key={stream.id} className="bg-white rounded-lg border p-4">
              ...
              <div className="grid grid-cols-3 gap-2 text-center">
                <div className="bg-gray-50 p-2 rounded">
                  <div className="text-lg font-bold text-blue-600">{stream?.conversationsToday || 0}</div>
                  <div className="text-xs text-gray-500">Convos Today</div>
                </div>
                <div className="bg-gray-50 p-2 rounded">
                  <div className="text-lg font-bold text-green-600">{(stream?.tokensProcessed || 0).toLocaleString()}</div>
                  <div className="text-xs text-gray-500">Tokens</div>
                </div>
                <div className="bg-gray-50 p-2 rounded">
                  <div className="text-lg font-bold text-purple-600">{stream?.projectsGenerated || 0}</div>
      ...
      <div className="bg-gradient-to-r from-indigo-50 to-purple-50 p-6 rounded-lg border border-indigo-200">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-white rounded-lg p-4 text-center">
            <div className="text-2xl font-bold text-blue-600">{(realTimeAnalytics?.totalTokens || 0).toLocaleString()}</div>
          ...
          <div className="bg-white rounded-lg p-4 text-center">
            <div className="text-2xl font-bold text-green-600">{realTimeAnalytics?.projectsGenerated || 0}</div>
          ...
          <div className="bg-white rounded-lg p-4 text-center">
            <div className="text-2xl font-bold text-purple-600">{realTimeAnalytics?.agentsDeployed || 0}</div>
          ...
          <div className="bg-white rounded-lg p-4 text-center">
            <div className="text-2xl font-bold text-orange-600">{realTimeAnalytics?.activeConnections || 0}</div>
            ...
            <tbody>
              {projects.map(project => (
                <tr key={project.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="text-center p-2">
                    <div className="font-medium text-green-600">{project?.metadata?.estimatedValue || 'N/A'}</div>
                    <div className="text-xs text-gray-500">Fit: {project?.metadata?.marketFit || 'N/A'}/10</div>
                  </td>
                  <td className="text-center p-2">
                    <div className={`inline-block px-2 py-1 rounded text-xs ${
                      (project?.metadata?.technicalRisk || 10) <= 3 ? 'bg-green-100 text-green-800' :
                      (project?.metadata?.technicalRisk || 10) <= 6 ? 'bg-yellow-100 text-yellow-800' :
                      'bg-red-100 text-red-800'
                    }`}>
                      {project?.metadata?.technicalRisk || 'N/A'}/10
                    </div>
                  </td>
                  <td className="text-center p-2">
                    <div className="text-xs">{project?.metadata?.timeToMarket || 'TBD'}</div>
                  </td>
                  <td className="text-center p-2">
            ...
            <div key={agent.id} className="bg-white rounded-lg border p-5">
              ...
              <div className="grid grid-cols-4 gap-3 mb-4">
                <div className="text-center bg-gray-50 rounded p-2">
                  <div className="text-sm font-bold text-blue-600">{(agent?.metrics?.processedConversations || 0).toLocaleString()}</div>
                  <div className="text-xs text-gray-500">Processed</div>
                </div>
                <div className="text-center bg-gray-50 rounded p-2">
                  <div className="text-sm font-bold text-green-600">{agent?.metrics?.projectsGenerated || 0}</div>
                  <div className="text-xs text-gray-500">Generated</div>
                </div>
                <div className="text-center bg-gray-50 rounded p-2">
                  <div className="text-sm font-bold text-purple-600">{agent?.metrics?.avgProcessingTime || 'N/A'}</div>
                  <div className="text-xs text-gray-500">Avg Time</div>
                </div>
                <div className="text-center bg-gray-50 rounded p-2">
                  <div className="text-sm font-bold text-orange-600">{agent?.metrics?.accuracy || 0}%</div>
              ...
              <div className="mt-3 pt-3 border-t text-xs text-gray-500">
                <div className="flex justify-between">
                  <span>Environment: {agent?.deployment?.environment || 'N/A'}</span>
                  <span>Uptime: {agent?.metrics?.uptime || 'N/A'}</span>
                </div>
                <div className="mt-1">
                  Last deployed: {agent?.deployment?.lastDeployment ? new Date(agent.deployment.lastDeployment).toLocaleString() : 'Never'}
                </div>
              </div>
            </div>
          ))}
    ...
    <div className="max-w-7xl mx-auto p-6 bg-gray-50 min-h-screen">
      <div className="mb-8">
        ...
        <div className="flex items-center gap-4 mt-2 text-sm text-gray-500">
          ...
          <span>{Object.values(apiConnections).filter(c => c.status === 'connected').length || 0} API connections active</span>
          <span>{(realTimeAnalytics?.totalTokens || 0).toLocaleString()} tokens processed</span>
        </div>
      </div>
